...
function New-BackendPane {
    $content = @(
        ...
        "Write-Host '=== BACKEND - $gpuDevice ===' -ForegroundColor Green",
        "Write-Host 'Validating model configurations...' -ForegroundColor Yellow",
        ...
        "Write-Host ''",
        "Write-Host 'Pre-validating providers...' -ForegroundColor Yellow",
        "& `"$pyExe`" -m app.cli validate-lvface",
        "& `"$pyExe`" -m app.cli validate-caption", 
        "Write-Host 'Starting backend with model loading...' -ForegroundColor Green",
    ...
    return @{ File = $path; Dir = $backendRoot; Title = "Backend ($gpuDevice)" }
}
