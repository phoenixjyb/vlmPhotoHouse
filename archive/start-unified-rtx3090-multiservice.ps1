...
if ($RunPreCheck) {
    ...
    try {
        ...
        & .\.venv\Scripts\python.exe gpu_precheck_validation.py | Out-Null
...
function New-CaptionModelsPane {
    ...
    $content = @(
        ...
        "# Do not set CUDA_VISIBLE_DEVICES - let PyTorch see both GPUs and use cuda:0 for RTX 3090",
        "`$env:TORCH_CUDA_ARCH_LIST = '8.6'",
        "",
        "# Test RTX 3090 caption inference",
        "Write-Host '🧪 Testing RTX 3090 caption capabilities...' -ForegroundColor Cyan",
        "& `"$pyExe`" -c `"",
        "import torch",
...
function New-LvfacePane {
    ...
    $content = @(
        ...
        "Write-Host 'ONNX Runtime + CUDA | Running inside Ubuntu-22.04' -ForegroundColor Yellow",
        "",
        "Set-Location -LiteralPath `"$LvfaceDir`"",
        "",
        "Write-Host '� Launching LVFace service via WSL...' -ForegroundColor Cyan",
        "wsl.exe -d Ubuntu-22.04 --cd `"$LvfaceDir`" bash start_wsl.sh"
    ) -join "`n"
    
    $path = Join-Path $env:TEMP "lvface-pane-$PID.ps1"
    Set-Content -LiteralPath $path -Value $content -Encoding UTF8
    return @{ File = $path; Dir = $LvfaceDir; Title = "LVFace (WSL Port 8003)" }
}
...
function New-TtsPane {
    ...
    $content = @(
        "Write-Host '�️ TTS Service - RTX 3090 Speech Synthesis' -ForegroundColor Green",
...
function New-GpuMonitoringPane {
    ...
    $content = @(
        ...
        "}",
        "",
        "# Fallback: Start AI orchestrator if monitoring fails",
