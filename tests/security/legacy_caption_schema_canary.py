...
with closing(sqlite3.connect(database.as_uri()+'?mode=ro', uri=True)) as check:
    ...
    assert check.execute('SELECT version_num FROM alembic_version').fetchall() == [('f2a6d8b4c915',)]
...
with ExitStack() as guards:
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            assert session.execute(text('SELECT version_num FROM alembic_version')).fetchall()==[('f2a6d8b4c915',)]
