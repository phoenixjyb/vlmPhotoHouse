...
class PeopleReviewTests(unittest.TestCase):
    ...
    def test_changed_manual_assignment_or_name_is_refused(self):
        with closing(sqlite3.connect(self.f.db)) as c, c:c.execute("UPDATE face_detections SET label_source='dnn' WHERE id=1")
        with self.assertRaisesRegex(ValueError,'assignment_changed'):self.export()
        with closing(sqlite3.connect(self.f.db)) as c, c:c.execute("UPDATE persons SET display_name='renamed' WHERE id=201")
