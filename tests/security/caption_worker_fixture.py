...
args = worker.argparse.Namespace(database=str(database), derived=str(work), temporary=str(work),
    ...
    expected_revision='a8d4c2e6f901', execute=True, legacy_worker_stopped=mode!='unconfirmed', once=mode!='drain')
...
with closing(sqlite3.connect(database)) as db:
    ...
    assert db.execute('SELECT version_num FROM alembic_version').fetchone()==('a8d4c2e6f901',)
    if mode=='drain':
