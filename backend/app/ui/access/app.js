...
(() => {
  ...
  async function openAsset(item,{sequence=[item],origin='single',advance=false,play=false}={}) {
    ...
    try {
      ...
      const [captions,url]=await Promise.all([request(libraryPath(`/assets/${item.id}/captions`),{epoch}),previewURL(item.id,epoch)]);
      ...
      return true;
    } catch(error) {if(viewerGeneration===state.viewerGeneration){stopSlideshow();$('view-quality').textContent=t(errorStatus(error));}await failure(error,epoch);return false;}
  ...
  async function restore(load=true) {
    ...
    try {
      ...
      if(stale(epoch))return;
      if(state.profile?.account_id!==profile.account_id){peopleState.page=1;peopleState.query='';$('people-query').value='';directoryState.page=1;directoryState.query='';$('directory-query').value='';tagState.page=1;tagState.query='';tagState.tag=null;tagState.open=null;$('tag-query').value='';$('tag-assets').replaceChildren();}
  ...
  async function openDiscovery(){
    if(state.locked||!state.library)return;
    const epoch=state.generation,library=state.library,load=++discoveryState.load;
  ...
  async function loadDiscovery(){
    ...
    const body={binding:discoveryState.binding,filters,page:discoveryState.page,page_size:24,
    ...
    try{
      ...
      if(!current())return;
      discoveryState.fingerprint=result.fingerprint;discoveryState.total=result.total;discoveryState.applied=true;
      ...
      if(error&&error.status===409){
        ...
        discoveryState.applied=false;discoveryState.binding=null;
        $('discovery-list').replaceChildren();$('discovery-pages').hidden=true;
