#!/usr/bin/env python3
"""Build a fixed, source-only caption worker ZIP from an explicit local commit.
...
FILES = tuple(sorted([
    *('backend/app/'+name+'.py' for name in ('__init__','config','db','tasks',
        'vector_index','image_utils','face_assignment_audit','metrics','gps_utils',
        'caption_policy','caption_service','tagging')),
...
def package_bytes(commit, files):
    ...
    manifest = {'format_version':1,'source_commit':commit,'artifact_kind':'caption_worker_source_only',
